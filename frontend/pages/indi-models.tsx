  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await fetch(
        apiUrl(`/girls/get-by-role/indi?offset=${offset}&limit=${LIMIT}&site_url=${encodeURIComponent(siteUrl)}`, false)
      );
      if (!res.ok) {
        setHasMore(false);
        setLoading(false);
...
export const getServerSideProps: GetServerSideProps<IndiModelsProps> = async ({ locale, req }) => {
  try {
    ...
    const [girlsRes, servicesRes, contactRes] = await Promise.all([
      fetch(apiUrl(`/girls/get-by-role/indi?offset=0&limit=${LIMIT}&site_url=${encodeURIComponent(siteUrl)}`, true)),
